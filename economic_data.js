...
const section_ec_button = document.querySelector('.section_ec_button');
...
const select_name_ec_container = document.querySelector('.select_name_ec_container');
const btn_form_ec = document.querySelector('.btn_form_ec');
...
const filterFn = async () =>{
    ...
    // const petitionFilter = await fetch(`http://localhost:3025/filter_news_calendar`,{
    //                 method:'POST',
    //                 body:JSON.stringify({
    //                     "from":`${fromFilter}`,
    //                     "to":`${toFilter}`
...
const viewDataNews = async () =>{
    ...
    const select_name_ec = document.createElement("select");
    ...
    select_name_ec.appendChild(option_form_name);
        //console.log(news);
    }

    select_name_ec_container.appendChild(select_name_ec)

    btn_form_ec.addEventListener("click",()=>{
        insertDataForm(select_name_ec)
    })
}

const viewNewsCalendar = async (show,quant) =>{
    
    let petition = await fetch(`http://localhost:3025/view_news_calendar`,{
                        method:'POST',
                        headers: {'Content-type':'application/json'}
...
section_ec_button.addEventListener("click",()=>{
    container_form_ec.style.display = "flex";
});

btn_form_ec.addEventListener("click",()=>{
    container_form_ec.style.display = "none";
})
